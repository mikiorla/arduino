...
GO_RIGHT = analogRead(RC_RIGHT); 
      ...
      if ((GO_FW > 1000) && (GO_RW < 100))//napred
            ...
            while ((GO_FW > 1000) && (GO_RW < 100))//napred
              ...
              if(fw==0)
              {
                for(;PWM_DUTY_FW<255-PWM_DIFF;PWM_DUTY_FW++)
                 {
                  
                  analogWrite(M2_Q1,PWM_DUTY_FW+PWM_DIFF);
      ...
      if (fw==1) //************************************************slow down from FW
        ...
        if  ((GO_RW > 1000) && (GO_FW < 100)) //nazad
              ...
              if(rw==0)
              {
                for(;PWM_DUTY_RW <255-PWM_DIFF;PWM_DUTY_RW++)
                 {
                  analogWrite(M2_Q4,PWM_DUTY_RW+PWM_DIFF);
